...
% %K-ilooa wszystkich genów opisanych terminem GO;
% %N-ilooa genów w klastrze;
% %x-ilooa skucesów = ilosc genów opisanycg w analizowanym klastrze term. GO;
% for j=1:10
...
fid=fopen('raport','w');
fprintf(fid,'Raport\n\n');
name='Wojciech Opoczynski';
fprintf(fid,'Imie i Nazwisko: %s \n',name);
fprintf(fid,'Data %s\n\n',datestr(now));
method='klastrowanie hierarchiczne';
fprintf(fid,'Metoda klastrowania: %s\n',method);
for j=1:10      %tutaj zaczyna sie obliczanie pVal na klastrowanie
    x=clusterGOTermsNo{j};
    M=size(chipgenes);
    M=M(1);
    N=length(clusters(clusters==j));
    ...
    pVal{j}=1-hygecdf(x-1,M,K,N);
%koniec obliczen pVal na klastry 
    term = sort(pVal{j});
    name=GOTerms{j};
